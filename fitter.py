...
def best_fit_distribution(data, bins=200, ax=None):
    ...
    DISTRIBUTIONS = [        
        ...
        # , st.levy_stable # this was literally taking 8 hours
        # , st.ncx2, st.ncf, st.nct, st.tukeylambda # These 4 were taking up >60% of the fit time
    ]

    # Best holders
    best_params = (0.0, 1.0)
    best_distribution = st.norm(*best_params)
    best_sse = np.inf
    ...
    for distribution in DISTRIBUTIONS:
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                # kse = st.kstest(data, dist.cdf).pvalue

                # # if axis pass in add to plot
                # try:
                #     if ax:
                #         pd.Series(dist.pdf(x), x).plot(ax=ax)
                # except Exception:
                #     pass
